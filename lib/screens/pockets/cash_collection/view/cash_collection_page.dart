...
class _CashCollectionPageState extends State<CashCollectionPage> {
  ...
  void initState() {
    ...
    context.read<CashCollectionBloc>().add(FetchCashCollectionByDateRange('', submissionStatus: 'pending'));
  }

  void _fetchCashCollectionForStatus(String status) {
    // Fetch cash collection with specific submission status only
    context.read<CashCollectionBloc>().add(FetchCashCollectionByDateRange('', submissionStatus: status));
  }
    ...
    return CustomScaffold(
      ...
      body: Column(
        children: [
          ...
          // Content
          Expanded(child: _buildTabContent(_submissionStatusOptions[_selectedTabIndex])),
        ],
      ),
    );
  }
        ...
        builder: (context, state) {
          if (state.fetchStatus == ApiStatus.loading) {
            ...
            return ErrorStateWidget(
              onRetry: () {
                context.read<CashCollectionBloc>().add(FetchCashCollectionByDateRange('last_1_day'));
              },
            );
            ...
            if (response?.data?.cashCollections == null || response!.data!.cashCollections!.isEmpty) {
              return _buildEmptyState();
            }

            // Filter by submission status
            final filteredCollections =
                response.data!.cashCollections!.where((collection) => collection.submissionStatus == status).toList();
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                CustomText(
                  text: CurrencyFormatter.formatAmount(context, collection.remainingAmount?.toString() ?? '0'),
  ...
  Widget _buildCashCollectionDetails(CashCollectionModel collection, bool isDarkTheme) {
    return Column(
      children: [
        _buildBreakdownRow(AppLocalizations.of(context)!.cashCollected, collection.cashCollected ?? '0', isDarkTheme),
        _buildBreakdownRow(AppLocalizations.of(context)!.cashSubmitted, collection.cashSubmitted ?? '0', isDarkTheme),
      ],
    );
  }
  ...
  Widget _buildBreakdownRow(String label, String amount, bool isDarkTheme) {
    return Padding(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          CustomText(text: label, fontSize: 12, color: Theme.of(context).colorScheme.onSurface.withValues(alpha: 0.7)),
          Expanded(
            child: CustomText(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8.h),
          CustomText(
            text: AppLocalizations.of(context)!.completeDeliveriesToSeeYourCashCollection,
