...
class EarningsBloc extends Bloc<EarningsEvent, EarningsState> {
  ...
  EarningsBloc(this._earningsRepo, {this.context})
      ...
      if (GlobalErrorHandler.is403Response(response)) {
        ...
        emit(
          state.copyWith(
            ...
            isInactive: true,
            message: apiMessage ?? 'Your account is currently inactive',
          ),
        );
        return;
      }
  ...
  Future<void> _onFetchEarningsStats(
    ...
    try {
      emit(
        state.copyWith(
          statsFetchStatus: ApiStatus.loading,
          clearMessage: true,
          isInactive: false,
        ),
      );
      ...
      if (response['success'] == true && response['data'] != null) {
        ...
        emit(
          state.copyWith(
            ...
            statsResponse: earningsStatsResponse,
          ),
        );
      } else {
        emit(
    ...
    try {
      ...
      if (response['success'] == true && response['data'] != null) {
        ...
        emit(
          state.copyWith(
            fetchStatus: ApiStatus.failed,
            message: 'Failed to load yearly earnings',
          ),
        );
      }
    } catch (error) {
