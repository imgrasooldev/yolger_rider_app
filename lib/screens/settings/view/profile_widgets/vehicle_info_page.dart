...
class _VehicleInfoPageState extends State<VehicleInfoPage> {
  ...
  void _populateFields(ProfileModel profile) {
    if (profile.deliveryBoy != null) {
      ...
      _tempDriverLicenseImages = profile.deliveryBoy!.driverLicense?.toList() ?? [];
      _tempVehicleRegistrationImages = profile.deliveryBoy!.vehicleRegistration?.toList() ?? [];
    }
  }
    ...
    if (_formKey.currentState!.validate()) {
      ...
      context.read<ProfileBloc>().add(
        UpdateProfile(
          ...
          driverLicenseFiles: _tempDriverLicenseImages.isNotEmpty ? _tempDriverLicenseImages : null,
          vehicleRegistrationFiles: _tempVehicleRegistrationImages.isNotEmpty ? _tempVehicleRegistrationImages : null,
        ),
      );
    }
  }
  ...
  List<String> _getCombinedDocumentUrls(String title, List<String>? existingUrls) {
    ...
    if (title.toLowerCase().contains('license')) {
      ...
      return _tempVehicleRegistrationImages.where((url) => url.isNotEmpty).toList();
    }
  ...
  bool _isLocalFile(String path) {
    return path.startsWith('/') || path.contains('\\') || (!path.startsWith('http://') && !path.startsWith('https://'));
  }
  ...
  Future<void> _pickAndUploadDocument(String documentType) async {
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        final filePaths = result.files.map((file) => file.path).where((path) => path != null).cast<String>().toList();
        ...
        ToastManager.show(
          context: context,
          message: '${filePaths.length} file(s) selected. Click "Save Changes" to upload.',
  ...
  void _removeSpecificImage(String imageUrl, int imageIndex) {
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: CustomText(text: 'Remove Image', fontSize: sz(16, seprateTabletSize: 12), fontWeight: FontWeight.bold),
          content: const CustomText(text: 'Are you sure you want to remove this image? This action cannot be undone.'),
          actions: [
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
              },
              child: CustomText(text: 'Cancel', fontSize: sz(14, seprateTabletSize: 10), color: Colors.grey),
            ),
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
              ...
              child: CustomText(text: 'Remove', fontSize: sz(12, seprateTabletSize: 9), color: Colors.red),
            ),
          ],
        );
      },
  ...
  void _performSpecificImageRemoval(String imageUrl, int imageIndex) {
    ...
    if (_tempDriverLicenseImages.contains(imageUrl)) {
      ...
      ToastManager.show(context: context, message: 'Document removed', type: ToastType.success);
      ...
      body: BlocConsumer<ProfileBloc, ProfileState>(
        ...
        listener: (context, state) {
          if (state.fetchStatus == ApiStatus.success &&
              ...
              state.documentUploadStatus == ApiStatus.initial) {
          ...
          } else if (state.documentUploadStatus == ApiStatus.loading) {
            setState(() {
              _isLoading = true;
            });
            if (state.documentType.isNotEmpty) {
              ToastManager.show(
                context: context,
                message: 'Uploading ${state.documentType.replaceAll('_', ' ')}...',
                type: ToastType.info,
              );
            }
          } else if (state.documentUploadStatus == ApiStatus.success) {
            setState(() {
              _isLoading = false;
            });
            if (state.documentType.isNotEmpty) {
              ToastManager.show(
                context: context,
                message: '${state.documentType.replaceAll('_', ' ')} uploaded successfully!',
                type: ToastType.success,
              );
            }
            context.read<ProfileBloc>().add(const LoadProfile());
        ...
        builder: (context, state) {
          if (state.fetchStatus == ApiStatus.success || state.updateStatus == ApiStatus.success) {
            if (state.profile != null) {
              return _buildVehicleInfoContent(state.profile!);
            }
          }
          ...
          return _buildVehicleInfoContent(state.profile ?? ProfileModel());
        },
      ),
    );
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildDocumentField(
                      ...
                      isLoading: profile.deliveryBoy == null, // Show loading when no delivery boy data
                    ),
                    SizedBox(height: 16.h),
                    ...
                    if (_isEditing) ...[
                      Row(
                        children: [
                          ...
                          Expanded(
                            child: CustomButton(
                              text: _isLoading ? 'Saving...' : AppLocalizations.of(context)!.saveChanges,
      ...
      child: Row(
        children: [
          ...
          CircleAvatar(
            ...
            child: Icon(Icons.directions_bike, size: (isTablet() ? 22 : 30).sp, color: AppColors.primaryColor),
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                // Vehicle Type / Title
                CustomText(
                  text: profile.deliveryBoy?.vehicleType ?? 'Vehicle Information',
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: (profile.deliveryBoy?.verificationStatus == 'verified' ? Colors.green : Colors.orange)
                        .withValues(alpha: isDarkTheme ? 0.3 : 0.15),
                    borderRadius: BorderRadius.circular(20.r),
                  ),
                  child: CustomText(
                    text: profile.deliveryBoy?.verificationStatus?.toUpperCase() ?? 'PENDING',
      ...
      children: [
        ...
        if (isEditing) ...[
          ...
          CustomCard(
            ...
            height: 48.h, // Match CustomTextFormField height (12+12+20+2+2 = 48)
            child: Row(
              children: [
                Icon(icon, color: AppColors.primaryColor, size: sz(18, seprateTabletSize: 14).sp),
                SizedBox(width: 10.w),
                Expanded(
                  child: CustomText(
                    text: controller.text.isEmpty ? 'Not provided' : controller.text,
        ...
        children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    if (!isLoading) ...[
                      CustomText(
                        text: hasDocument ? 'Document uploaded' : 'No document uploaded',
                        fontSize: sz(14, seprateTabletSize: 10),
            ...
            CustomCard(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(
                    height: 150.h, // Fixed height for horizontal ListView (150 + 20 for margins)
                    child: _buildMultipleImages(_getCombinedDocumentUrls(title, allUrls), isDarkTheme),
                  ),
                ],
              ),
            ),
          ...
          Row(
            children: [
              ...
              if (_isEditing) ...[
                CustomButton(
                  text: hasDocument ? 'Upload New' : AppLocalizations.of(context)!.uploadDocument,
                  onPressed: () {
                    final documentType =
                        title.toLowerCase().contains('license') ? 'driver_license' : 'vehicle_registration';
  ...
  Widget _buildMultipleImages(List<String> imageUrls, bool isDarkTheme) {
    ...
    final List<String> validUrls = imageUrls.where((url) => url.isNotEmpty).toList();
